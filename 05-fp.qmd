...
or failure and force you to handle both cases. We will explore this in the
chapter on monads, where we introduce `{chronicler}` for R and `talvez` for
Python.
...
In the next chapter, we'll put these principles into practice with `{rixpress}`,
a package that leverages functional composition and Nix to build fully
reproducible, polyglot analytical pipelines.
