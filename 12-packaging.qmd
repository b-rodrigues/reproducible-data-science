...
First, make sure you have the necessary tools installed. Create a Nix
environment that contains the following R packages: `{devtools}`, `{usethis}`
and `{roxygen2}`.

Drop into this Nix shell, and let `{usethis}` create the package structure for
you. From your R console, run:
...
If you want to create an environment using Nix that includes this package, use
the `git_pkgs` argument of `rix::rix()` to generate the right `default.nix`
file.
...
With this, dropping into the Nix shell, starting the Python interpreter and then
typing `import pyclean` will work without any issues. You may need to adapt the
path depending on where you're developing the package.
...
You need to add the `rev`, which corresponds to the commit that you want, and the
...
`sha256`. Also note that this isn’t the most idiomatic way to build a Python
