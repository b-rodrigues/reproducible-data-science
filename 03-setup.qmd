...
If you have `qemu` installed, you can automatically download Lubuntu 24.04 
and start a VM with 4 GB of RAM on a persistent disk of 20 GB. 
...
ARCH=$(uname -m)
ISO_X86="$VM_DIR/lubuntu.iso"
ISO_ARM="$VM_DIR/lubuntu-arm64.iso"
DISK="$VM_DIR/lubuntu.qcow2"

# Create persistent disk if it doesn't exist
[ ! -f "$DISK" ] && qemu-img create -f qcow2 "$DISK" 20G

# Download appropriate Lubuntu ISO if missing
if [ "$ARCH" = "x86_64" ]; then
  [ ! -f "$ISO_X86" ] && curl -L -o "$ISO_X86" https://cdimage.ubuntu.com/lubuntu/releases/24.04/release/lubuntu-24.04-desktop-amd64.iso
  qemu-system-x86_64 -enable-kvm -m 4096 -hda "$DISK" -cdrom "$ISO_X86" -boot c
elif [ "$ARCH" = "arm64" ]; then
  [ ! -f "$ISO_ARM" ] && curl -L -o "$ISO_ARM" https://cdimage.ubuntu.com/lubuntu/releases/24.04/release/lubuntu-24.04-desktop-arm64.iso
  qemu-system-aarch64 -accel hvf -machine virt -cpu cortex-a72 -m 4096 -hda "$DISK" -cdrom "$ISO_ARM" -boot c
else
