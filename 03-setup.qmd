...
### installing Nix (All Systems)

Installing (and uninstalling) Nix is quite simple, thanks to the installer from
[Determinate Systems](https://github.com/DeterminateSystems/nix-installer), a company
...
1. Start a temporary shell with R and `{rix}` again using
  ...
  Run the `available_dates()` function to see the list of available snapshots.
2. Inside of an activated shell, type `which R` and `echo $PATH`. Explore what
  ...
  `/nix/store/...`?
3. Go to [https://search.nixos.org/packages](https://search.nixos.org/packages)
