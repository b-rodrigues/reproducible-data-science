...
the operating system (but requires WSL2 on Windows). If you are familiar with
the Ubuntu Linux distribution, you have likely used `apt-get` to install
software. On macOS, you may have used `homebrew` for similar purposes. Nix
functions in a comparable way but has many advantages over classic package
managers, as it focuses on reproducible builds and downloads packages from
...
But in practice, one or more of these bullet points are missing from projects.
Our goal is to learn how to fulfil all the requirements to build reproducible
projects.
...
your project doesn’t make it reproducible if you don’t explicitly take some
precautionary steps.
...
([source](https://nix.dev/manual/nix/2.25/language/derivations)^[https://nix.dev/manual/nix/2.25/language/derivations])
...
let
  pkgs = import (
   fetchTarball "https://github.com/rstats-on-nix/nixpkgs/archive/2025-04-11.tar.gz"
  ) {};
in
...
The goal of the tool we will use in this book, T, is to help you create
pipelines from such derivations without needing to learn the Nix language
itself, while still benefiting from its powerful reproducibility features.
...
- **Flakes**: The newer way to define and pin Nix projects. Flakes make it
  ...
  is a lot of discussion around flakes, as they're officially still considered
...
As a little sidenote: I also want to highlight that Nix can even be used to
...
`home-manager`.^[https://github.com/nix-community/home-manager] This is outside
...
- **Disk space and builds**: Because Nix never mutates software in place, the
  ...
  to set up your own project cache if you so wish. Setting up your own cache
