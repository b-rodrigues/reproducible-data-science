...
You can now use several languages for a project: if you’re a Python user, you now
can use R’s `{ggplot2}` quite easily by simply adding R and `{ggplot2}` to your
project using Nix. However, in a way, this is just moving goalposts: as annoying
as it was to set up R and `{ggplot2}`, and no matter how easily Nix solves this,
now you need to set up Nix and get familiar with it!

But thankfully we live in AI age, and LLMs can be quite helpful here. My goal is
for you to not have to have to worry (too much) about setting up projects
and making them reproducible thanks to LLMs and a new programming language
made specifically for setting up reproducible pipelines, T.
...
The third reason is that we actually can offload so much of this to LLMs that
