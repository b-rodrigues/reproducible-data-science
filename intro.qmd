...
worry (too much) about project setup, letting LLMs and T handle the heavy lifting.
...
organisations. It's that time of the year again, and a report needs to be written. Ten people are
...
dependency pinned and documented, and still be practically irreproducible because
nobody can figure out which script to run first, in what order, and with what
inputs. Environment reproducibility is necessary but not sufficient; you also
need what we might call *computational reproducibility*: an explicit, runnable
description of the pipeline itself, not just the software it runs on. We will
return to this distinction in Chapter 4, where it becomes the central motivation
for T's mandatory pipeline architecture.
