...
As pipelines grow, a single `gen-pipeline.R` can become hard to navigate and
collaborate on. `{rixpress}` includes `rxp_pipeline()` to group derivations into
named sub-pipelines and combine them into a master pipeline.
...
The master script stays concise by pointing to those files:
...
When pipelines are defined, visualisation tools use pipeline metadata:

- `rxp_visnetwork()` colours nodes by pipeline group.
- `rxp_ggdag()` defaults to dual encoding (fill = derivation type, border = pipeline).
- `rxp_trace()` uses pipeline colours in the console.

Switch colour modes as needed:
...
# Color by derivation type
rxp_ggdag(color_by = "type")
```

Sub-pipelines keep large workflows modular while preserving metadata for DAG
